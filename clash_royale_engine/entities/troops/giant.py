"""Giant troop — slow, tanky, targets buildings only."""
...
def create_giant(player_id: int, x: float, y: float, fps: int = DEFAULT_FPS) -> Entity:
    """Factory function for a Giant entity."""
    s = CARD_STATS["giant"]
    return Entity(
        name=s["name"],
        player_id=player_id,
